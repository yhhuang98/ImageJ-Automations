...
fileName = "NT125N-24h-Z-Series";
sourceDir = "C:/Confocal_raw_data/";
saveDir = "C:/Confocal_raw_data/save_figs/";
...
for (i=0;i<5;i++){
	...
	while (avgSize < 100 || avgSize > 150){
		...
		run("Analyze Particles...", "size=20-1000 clear summarize overlay");
	    ...
	    else if (avgSize > 150 && minThreshold < 255) {
	        minThreshold += 10;
	        Table.deleteRows(Table.size-1, Table.size-1, "Summary");
	        //close("Thresholded-"+iter);
	    }
